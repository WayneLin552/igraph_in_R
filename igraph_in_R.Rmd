...
l <- layout_with_fr(net_pc) 
plot(net_pc, 
     vertex.color = 'red',                    # The color of nodes
     vertex.frame.color = 'lightblue',        # The color of nodes' frame
     vertex.shape = c('circle','rectangle'),  # The color of nodes' frame
     vertex.size = 25,                        # The size of a vertex
...
plot(net_pc, 
     ...
     vertex.label.dist = 0.5,            # Change the distance between labels and nodes' frames
...
plot(karate, 
     ...
     layout  = l)
```
Finally, we divide the edges into three categories and use different colors for them. Remember the karate club is splitted into two factions, so the edges can be divided into: edges inside faction 1, edges inside faction 2, and edges connecting both factions. \
...
E(karate)[F1 %--% F1]$color<-"forestgreen"
